...
class rules:
    def __init__(self):
        ...
        self.color = {-1 : (240,240,240), 0 : (255,0,0), 1 : (0,255,0)}
...
class gridmap:
    ...
    def resetMask(self, mask):
        for pos in mask:
            if (pos[0] >= self.width or pos[1] >= self.width):
                return
            self.grid[*pos] = -1
...
def rotateMask(currentAngle, radius, brush, width):
    ...
    return getBrushMask([x,y], brush), [x,y]
...
brush = loadBrush("brushSolid.png")
brushSmall = loadBrush("5block.png")
...
while running:
    ...
    ## first
    mask, pos = rotateMask(ang*math.pi/180, 45, brush, m.width)
    m.updateSingle(pos, r)
    ...
    mask, pos = rotateMask((ang+10)*math.pi/180, 45, brush, m.width)
    m.resetMask(mask)
    
    ### second
    mask, pos = rotateMask((ang+180)*math.pi/180, 45, brush, m.width)
    m.updateSingle(pos, r)
    boarder = m.getBoarders()
    selectionRange = list(set(map(tuple, boarder)) & set(map(tuple, mask)))
    if len(selectionRange) != 0:
    ...
    mask, pos = rotateMask((ang+10+180)*math.pi/180, 45, brush, m.width)
    m.resetMask(mask)

    #### smaller circ ############### smaller circ ############### smaller circ ############### smaller circ ############### smaller circ ###########
    speed = -3
    mask, pos = rotateMask((speed*ang)*math.pi/180, 20, brushSmall, m.width)
    m.updateSingle(pos, r)
    ...
    mask, pos = rotateMask(((speed*ang)-10)*math.pi/180, 20, brushSmall, m.width)
    m.resetMask(mask)
    
    ### second
    mask, pos = rotateMask(((speed*ang)+180)*math.pi/180, 20, brushSmall, m.width)
    m.updateSingle(pos, r)
    boarder = m.getBoarders()
    selectionRange = list(set(map(tuple, boarder)) & set(map(tuple, mask)))
    if len(selectionRange) != 0:
    ...
    mask, pos = rotateMask(((speed*ang)+170)*math.pi/180, 20, brushSmall, m.width)
    m.resetMask(mask)
    ...
    ###screenshot screen
    pg.image.save(screen, "imagetemp.png")
    imgtemp = cv2.imread("imagetemp.png")
    cv2.imwrite("image.png", imgtemp)
